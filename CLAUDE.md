...
- 자동화: GitHub Actions (`.github/workflows/update.yml`) · 스케줄 10개
  - 평일 08:00 아침 브리핑 (+08:25 예비) / 12:05 점심 맥박 (+12:35 예비)
    / 14:50 종가 스캔 (+15:07 예비) / 19:10 수집+마감요약 (+19:40 예비)
  - 일요일 18:00 주간 결산 · 평일 21:30 🩺 야간 자가 점검(마감 수집 반영 감시)
  - 모든 예비 알람은 본 실행 완료(발송 장부·데이터 날짜·scans 날짜)를 확인하면 아무것도 안 함
  - 어떤 단계든 실패하면 텔레그램 자동 통보 + 결과물이 이상하면 야간 점검이 통보
    (즉 "조용한 실패"까지 감시망에 포함)
