...
참고: 아침 브리핑은 08:25, 저녁 수집은 19:40에 예비 알람이 한 번 더 시도하므로
...
### "오프라인 테스트" 단계가 빨강 (N/59 passed)

테스트가 수집을 차단한 것 (의도된 안전장치). FAIL 난 테스트 이름을 로그에서 찾아
AI에게 전달. **AI 규칙**: 테스트는 반드시 고정 픽스처만 사용 — 저장소의 실제
...
### 예약 시간이 됐는데 실행 자체가 없음 (Actions에 새 줄이 안 생김)

GitHub 무료 예약의 지연/누락 — 우리 잘못 아님 (2026-07-13 실제: 종가 스캔 2시간 46분
지연, 저녁 수집 통째로 누락). 대책은 이중 알람(19:40 예비 수집, update.yml에 내장).
